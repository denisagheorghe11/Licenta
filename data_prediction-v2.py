...
# acum datele sunt deja stocate (în dataframe) si mai departe vom crea coloana 
# target care va contine doar valori de 1 și 0, functie de coloana y
df['target'] = df['y'].apply(lambda x: 1 if x == 'yes' else 0)
# ultima coloana reprezinta coloana target, considerand că în analiză vom lua în
# considerare doar valorile rezultate atunci cand telefonul este conectat la rețea
# S-ar putea sa existe exporturi de parametri atunci cand nu exista un echipament tinta.
# In acest caz, vom elimina aceste inregistrari deoarece nu sunt de interes.
#
...
df.describe()
#df.dtypes.to_excel(os.path.join(cd, '{0}.xlsx'.format('types')), index=False)
...
#atatea campuri am de tip diferit de 'obiect'
...
#features.to_excel(os.path.join(cd, '{0}.xlsx'.format('features')), index=False)
...
def mono_bin(Y, X, n = max_bin):
    ...
    while np.abs(r) < 1:
        try:
            ...
            # pd.qcut(notmiss.X, n) împarte în grupuri de cate n valori, valorile diferite de 0 ale lui X
            d2 = d1.groupby('Bucket', as_index=True)
            r, p = stats.spearmanr(d2.mean().X, d2.mean().Y)
# spearman reprezinta functia de corelatie cu acelasi nume si este
# o masura neparametrica a monotoniei relatiei dintre doua seturi de date
# valoarea lui p indica probabilitatea ca un sistem necorelat sa proca seturi
# de date care au corelatia Spearman la fel de extrema ca cea compusa din aceste seturi de date
            n = n - 1
        except Exception:
            n = n - 1
    ...
    # woe - weight of evidence = metodă de evaluare a predictorilor
...
final_iv.to_excel(os.path.join(cd, '{0}.xlsx'.format('final_IV')), index=False)
print (final_iv)

#selecția variabilelor și pregătirea datelor

IV = IV.rename(columns={'VAR_NAME':'index'})
IV = IV.sort_values(['IV'],ascending=0)

IV.to_excel(os.path.join(cd, '{0}.xlsx'.format('index_IV')), index=False)
...
VI.to_excel(os.path.join(cd, '{0}.xlsx'.format('index_RF')), index=False)
...
FI = FI.reset_index()
FI = FI.sort_values(['Extratrees'],ascending=0)

FI.to_excel(os.path.join(cd, '{0}.xlsx'.format('index_Extratress')), index=False)
...
chi_sq.to_excel(os.path.join(cd, '{0}.xlsx'.format('index_Chi_Square')), index=False)
...
final_results.to_excel(os.path.join(cd, '{0}.xlsx'.format('final_results')), index=False)
...
score_table.sort_values('final_score',ascending=0).to_excel(os.path.join(cd, '{0}.xlsx'.format('final_score')), index=False)
...
# impartirea datelor in date de invatare si date de test
...
features_train.to_excel(os.path.join(cd, '{0}.xlsx'.format('features_train')), index=False)
...
print("Precizia pentru datele de învățare: ",accuracy_train*100, "%")
print("Precizia pentru datele de test: ",accuracy_test*100, "%")
print("Acoperirea zonei de sub curba pentru datele de invatare: ",auc_train*100, "%")
print("Acoperirea zonei de sub curba pentru datele de test: ",auc_test*100, "%")
...
#aplicarea algoritmului GradientBoosting
...
#aplicarea algoritmului LogisticRegression
from sklearn.linear_model import LogisticRegression
...
print("Random Forests - Hyper parameter tuning:")
...
plt.plot(fpr,tpr,color='#0077bc',label = 'AUC = '+ str(round(auc,3)))
...
lift_train.to_excel(os.path.join(cd, '{0}.xlsx'.format('lift_train')), index=False)
